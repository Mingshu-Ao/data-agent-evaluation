...
from data_agent_baseline.tools.context_profile import profile_context
...
from data_agent_baseline.tools.structured_sql import execute_structured_sql
...
def _profile_context(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    ...
    max_doc_chars = int(action_input.get("max_doc_chars", 1200))
    return ToolExecutionResult(
        ok=True,
        content=profile_context(task, sample_rows=sample_rows, max_doc_chars=max_doc_chars),
    )
...
def _execute_structured_sql(
    ...
    return ToolExecutionResult(
        ok=True,
        content=execute_structured_sql(task.context_dir, sql, limit=limit),
    )
...
def _answer(_: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    ...
    if not isinstance(rows, list):
        raise TypeError("answer.rows must be a list.")

    normalized_rows: list[list[Any]] = []
    for row in rows:
        if not isinstance(row, list):
            raise TypeError("Each answer row must be a list.")
...
def create_default_tool_registry() -> ToolRegistry:
    specs = {
        ...
        "execute_structured_sql": ToolSpec(
            name="execute_structured_sql",
            description=(
                "Load task-local SQLite, JSON, CSV, and Parquet sources into one temporary DuckDB "
                "database, then run one read-only SQL query, including cross-source joins. Use "
                "SQLite table names, JSON `table` values, and CSV/Parquet file stems as table names."
            ),
        ...
        "profile_context": ToolSpec(
            ...
            description=(
                "Summarize all context files before planning. Returns CSV columns and row counts, "
                "JSON structures, sqlite schemas, and short document previews."
            ),
            input_schema={"sample_rows": 3, "max_doc_chars": 1200},
        ),
        "read_csv": ToolSpec(
            name="read_csv",
        ...
        "read_doc": ToolSpec(
            name="read_doc",
            description="Read a Markdown, text, or PDF document inside context.",
            input_schema={"path": "relative/path/to/file.md_or_pdf", "max_chars": 4000},
        ),
        "read_json": ToolSpec(
            name="read_json",
    ...
    handlers = {
        ...
        "execute_structured_sql": _execute_structured_sql,
